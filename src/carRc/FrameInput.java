...
public class FrameInput {
    ...
    public static final int RIDE_BACKWARD_RIGHT=8;
    public static final int RIDE_BACKWARD_LEFT=9;
    ...
    public static final int GET_STATUS_ALL_STRUCTURE=130;
        ...
        //System.out.print(String.format(" %X ",data));
         if(frameInput.size()>0){ //Sprawdzenie czy w buforze znajują się jakieś dane
             ...
             if(data==0xFF){ //Sprawdzenie czy odebrano 0xFF( sygnalizując nową ramkę)
                 //System.out.println(String.format("Bit przed %X ",frameInput.get(frameInput.size()-1)));
                 if(frameInput.get(frameInput.size()-1)!=0xFF) {
                    frameInput.add(data);
                }
                 return false;
             }
